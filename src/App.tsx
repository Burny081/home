...
export default function App() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      if (data) {
        // Merge Supabase with Mock PRODUCTS for a richer UI
        // Filter out One Piece from both sources just in case it's in DB
        const dbProducts = data.filter((p: any) => p.category !== 'onepiece');
        const mockProducts = PRODUCTS.filter(p => p.category !== 'onepiece');

        // Combine them, prioritizing Supabase items if names match
        const dbNames = new Set(dbProducts.map(p => p.name));
        const uniqueMock = mockProducts.filter(p => !dbNames.has(p.name));

        setLiveProducts([...dbProducts, ...uniqueMock]);
      } else {
        // If DB is empty or fails, use default mock data (filtered)
        setLiveProducts(PRODUCTS.filter(p => p.category !== 'onepiece'));
      }
    };
  ...
  const updateProduct = async (p: Product) => {
    ...
    if (error) {
      showToast('Action failed: Could not update product', 'info');
      return;
    }
  ...
  const deleteProduct = async (id: number) => {
    ...
    if (error) {
      showToast('Action failed: Could not delete product', 'info');
      return;
    }

    setLiveProducts(prev => prev.filter(item => item.id !== id));
    if (p) showToast('Product removed from inventory', 'info');
  };
  ...
  const addToCart = (product: Product, qty: number = 1) => {
    setCartItems(prev => {
      ...
      if (currentQty + qty > product.stock) {
        showToast('Inventory limit reached for this item', 'info');
        return prev;
      }
  ...
  const reduceStock = async (items: CartItem[]) => {
    const updates = items.map(async (item) => {
      ...
      if (newStock === 0) {
        await supabase.from('notifications').insert([{ message: `STOCK ALERT: ${item.product.name} is now out of stock!`, type: 'warning' }]);
      }
  ...
  const toggleFavorite = (id: number) => {
    setFavorites(prev => {
      ...
      if (next.has(id)) {
        next.delete(id);
        if (product) showToast('Removed from favorites');
      } else {
        next.add(id);
        if (product) showToast('Added to favorites');
      }
      return next;
    });
  };
  ...
  const handleLockAdmin = async () => {
    ...
    setCurrentView('home');
    showToast('Vault Secured ­ƒöÆ', 'info');
  };
    ...
    <div className="mb-24">
      ...
      {favorites.size > 0 ? (
        ...
        <div className={`flex flex-col items-center justify-center py-20 rounded-[2rem] border-2 border-dashed ${theme === 'dark' ? 'bg-white/5 border-white/10 text-slate-400' : 'bg-gray-50 border-gray-200'}`}>
          ...
          <button onClick={() => setCurrentView('shop')} className="mt-4 text-blue-600 font-bold hover:underline">Browse the shop ÔåÆ</button>
        </div>
      )}
    </div>
  );
    ...
    <div className="mb-20">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {[
          ...
          { bg: 'bg-blue-600', icon: <BarChart3 className="w-20 h-20" />, label: 'Active Assets', value: '42 Items', sub: '12 Graded ÔÇó 30 Sealed', subColor: 'text-white/80' },
        ].map((c, i) => (
    ...
    <div className={`min-h-screen font-sans antialiased overflow-x-hidden selection:bg-blue-100 selection:text-blue-900 transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'}`} data-theme={theme}>
      ...
      <div className="relative flex min-h-screen w-full flex-col">
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-8">
            <div className="flex items-center justify-between h-16 md:h-20">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setCurrentView('cart')} className={`p-2.5 rounded-xl transition-all active:scale-95 group relative ${theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-gray-100'}`}>
                  <ShoppingBag className={`w-6 h-6 ${currentView === 'cart' ? 'text-blue-500' : theme === 'dark' ? 'text-slate-400' : 'text-slate-700'} group-hover:text-blue-500`} />
                  {cartItems.length > 0 && (
          ...
          {currentView !== 'admin' && (
            <div className="max-w-3xl mx-auto px-4 pb-3">
              ...
              <motion.button
                ...
                className={`mt-3 w-full py-2.5 rounded-xl text-xs font-black uppercase tracking-[0.2em] transition-all active:scale-90 shadow-lg border ${theme === 'dark' ? 'bg-blue-600 border-blue-500 text-white shadow-blue-500/20 hover:bg-blue-500' : 'bg-blue-600 border-blue-500 text-white shadow-blue-400/20 hover:bg-blue-700'}`}
              >
                Let's Shop
              </motion.button>
            </div>
        ...
        <main className="flex-1 pb-20 md:pb-0">
          <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 pb-20 md:py-10">
        ...
        {/* Mobile Bottom Nav */}
        <nav className={`fixed bottom-0 left-0 w-full md:hidden z-50 border-t backdrop-blur-xl transition-all ${theme === 'dark' ? 'bg-slate-950/90 border-white/10' : 'bg-white/95 border-gray-100'}`}>
            ...
            {[
              ...
              return (
                <button key={item.id}
                  ...
                  className={`flex flex-col items-center justify-center gap-1 w-full transition-all active:scale-90 ${isActive ? (theme === 'dark' ? 'text-[#00f0ff]' : 'text-blue-600') : (theme === 'dark' ? 'text-slate-500' : 'text-slate-400')}`}
                >
                  <div className="relative">
                    <item.icon className={`w-5 h-5 ${isActive ? 'fill-current opacity-20' : ''}`} />
                    {(item.id === 'favorites' && favorites.size > 0) ? (
                      <span className={`absolute -top-1.5 -right-1.5 flex h-3 w-3 items-center justify-center rounded-full text-[8px] font-black text-white border border-slate-950 bg-red-500`}>
                        {favorites.size}
                      </span>
                    ) : null}
                  </div>
              ...
              >
                ...
                <div className="flex flex-col gap-1 flex-1">
                  {[
                    ...
                    { icon: ShoppingBag, label: `My Cart (${cartCount})`, id: 'cart', highlight: cartCount > 0 },
                    ...
                    <button key={idx}
                      ...
                      className={`flex items-center justify-between p-3.5 rounded-2xl transition-all group ${item.highlight ? 'bg-blue-600/10' : theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-blue-50'}`}
                    >
                      <div className="flex items-center gap-3">
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className={`fixed bottom-24 left-1/2 z-[100] border-2 px-6 py-4 rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] flex items-center gap-5 min-w-[320px] max-w-[90vw] transition-all ${theme === 'dark' ? 'bg-slate-900/90 backdrop-blur-2xl border-white/10 shadow-blue-500/20' : 'bg-white/90 backdrop-blur-2xl border-blue-50 shadow-blue-500/10'}`}
          >
            <div className={`flex-shrink-0 w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${theme === 'dark' ? 'bg-blue-600/20 ring-1 ring-blue-500/30' : 'bg-blue-50 ring-1 ring-blue-100'}`}>
              <Check className={`w-6 h-6 ${toast.type === 'info' ? 'text-amber-500' : 'text-blue-500'}`} />
            </div>
            <div className="flex-1 min-w-0">
              <p className={`text-[13px] font-black uppercase tracking-widest ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'} mb-0.5`}>
                {toast.type === 'info' ? 'Information' : 'Success'}
              </p>
              <p className={`text-base font-bold tracking-tight leading-tight truncate ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{toast.message}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
